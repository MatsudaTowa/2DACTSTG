...
void CCharacter::ShotBullet(D3DXVECTOR3 pos, D3DXVECTOR3 size, bool bWay,int nDamage,CBullet::BULLET_TYPE type)
{
	if (bWay == true)
	{//右向き
		CBullet* pBullet = CBullet::Create(D3DXVECTOR3(pos.x, pos.y , pos.z), D3DXVECTOR3(sinf(GetRot().y + D3DX_PI) * 3.0f, 0.0f, cosf(GetRot().y + D3DX_PI) * 3.0f),
			D3DXVECTOR3(0.0f, 0.0f, GetRot().y * 2.0f), D3DXVECTOR3(size.x, size.y, 0.0f), 60, nDamage, type);
	}
	else if (bWay == false)
	{//左向き
		CBullet* pBullet = CBullet::Create(D3DXVECTOR3(pos.x, pos.y, pos.z), D3DXVECTOR3(sinf(GetRot().y + D3DX_PI) * 3.0f, 0.0f, cosf(GetRot().y + D3DX_PI) * 3.0f),
			D3DXVECTOR3(0.0f, 0.0f, GetRot().y * 4.0f), D3DXVECTOR3(size.x, size.y, 0.0f), 60, nDamage, type);
	}
}
